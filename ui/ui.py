...
st.markdown("""<style>@keyframes glowSweep{0%{background-position:-250px 0;}100%{background-position:250px 0;}}.glow-divider{width:140px;height:1px;margin:12px 0 20px 0;background:linear-gradient(90deg,rgba(255,180,80,.95) 0%,rgba(255,180,80,.8) 20%,rgba(255,220,140,1) 35%,rgba(255,180,80,.45) 65%,rgba(255,180,80,.15) 100%);background-size:300px 100%;animation:glowSweep 2.2s linear infinite;box-shadow:0 0 6px rgba(255,180,80,.45),0 0 10px rgba(255,180,80,.25);}</style><div class="glow-divider"></div>""", unsafe_allow_html=True)
...
def render_batch_intelligence():
    ...
    with c2:
        ...
        with st.container(border=True):
            ...
            if st.session_state.completed_job_data:
                cached_insight_job_id = st.session_state.get("cached_insights_job_id")
                ...
                if cached_insight_job_id!=st.session_state.job_id:
                    ...
                    for _ in range(10):
                        ...
                        if raw_insights:
                            ...
                            st.session_state.cached_insights_job_id = st.session_state.job_id
                            break
