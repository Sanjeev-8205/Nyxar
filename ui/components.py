...
def inference_output_card(
    ...
    if sentiment is None:
        ...
        with st.container(border=True):
            ...
            st.markdown(f"""<div style="border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:1rem;background:linear-gradient(180deg,rgba(17,24,39,0.96),rgba(8,12,24,0.96));margin-bottom:1rem;text-align:center;"><div style="color:#9CA3AF;font-size:0.82rem;margin-bottom:0.75rem;">Confidence Score</div><svg width="180" height="100" viewBox="0 0 180 100"><path d="M20 90 A70 70 0 0 1 160 90" fill="none" stroke="rgba(255,255,255,0.12)" stroke-width="12" stroke-linecap="round"/><path d="M20 90 A70 70 0 0 1 160 90" fill="none" stroke="{'#10B981' if confidence>=0.7 else '#F59E0B' if confidence>=0.4 else '#EF4444'}" stroke-width="12" stroke-linecap="round" pathLength="100" stroke-dasharray="{confidence*100} 100"/><text x="90" y="70" text-anchor="middle" fill="#F3F4F6" font-size="24" font-weight="700">{confidence:.0%}</text></svg></div>""", unsafe_allow_html=True)
