...
def inference_output_card(
    ...
    if sentiment is None:
        ...
        with st.container(border=True):
            ...
            st.markdown("""<div style="color:#9CA3AF;font-size:0.92rem;margin-bottom:0.7rem;">Live prediction intelligence</div>""", unsafe_allow_html=True)

            st.markdown(f"""<div style="color:{sentiment_color};font-size:3rem;font-weight:800;letter-spacing:0.03em;margin-bottom:0.7rem;">{sentiment.upper()}</div>""", unsafe_allow_html=True)
